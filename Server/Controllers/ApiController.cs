...
public class ApiController : Controller
{
    ...
    [Route("{algorithm}/api/v1/videos/{videoId}")]
    [HttpGet]
    public async Task<VideoResponse> GetVideo([FromRoute] string algorithm, [FromRoute] string videoId)
    {
        var videoResponse = await _invidiousAPIAccess.GetVideo(videoId);
        var recommendedVideos = _algorithmAccess.GetAlgorithmVideos(algorithm, videoResponse.RecommendedVideos.Count());
        videoResponse.RecommendedVideos = recommendedVideos;
        return videoResponse;
    }

    [Route("{algorithm}/api/v1/search")]
    [HttpGet]
    ...
    [Route("{algorithm}/api/v1/channels/{channelId}")]
    [HttpGet]
    ...
    [Route("{algorithm}/api/v1/channels/{channelId}/videos")]
    [HttpGet]
