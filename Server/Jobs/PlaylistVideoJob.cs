...
public class PlaylistVideoJob : IJob
{
    ...
    private async Task UpdatePlaylistVideos(VideoDbContext videoDbContext, PlaylistEntity playlist)
    {
        ...
        {
            //The playlistVideo has a videoId, but we need a UniqueId to get the Index from response.Videos
            ...
            playlistVideo.Index = response.Videos.First(z => z.VideoId == uniqueId).Index;
        }
        playlist.DateLastScraped = DateTime.UtcNow;
        videoDbContext.SaveChanges();
        if (videosToAdd.Any())
